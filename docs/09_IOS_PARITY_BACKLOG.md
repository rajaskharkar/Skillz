...
- Extracts Scyra visual language: Royal Manuscript Blue / Scyra primary blue, current Android teal drift, Caveat-style title, top bar order, cards, chips, Shell rooms, reward reveal, The Blue, The Chest, Badges, Focus Room.
...
- Identifies visual risks around font licensing, procedural creatures, localization layout, and custom SwiftUI top bar requirements.
...
- Recommends SwiftData or SQLite behind repositories, a transaction/unit-of-work boundary, in-memory stores, and iOS domain/persistence model names.
...
- Documents blockers: movement ratio (`steps / 100` current Android vs possible product direction), Surge ambiguity, Creature Mastery idempotency gap, legacy reward categories.
...
- Drives AppIcon/LaunchScreen, font/license, turtle/logo provenance, String Catalog, localization review, The Blue redraw, and accessibility-label tasks.
...
- `ios/Scyra/Resources/Fonts/caveatsb.ttf` only after license verification and copied locally.
...
- Scyra-first. Aera support is deferred unless explicitly scoped.
...
- Legacy objects/trinkets/discoveries/Discovery Journal/Coral/Plants/Beam should not be ported to iOS MVP without product confirmation.
...
## 5. Recommended iOS MVP Scope

### Must Have for iOS MVP

- iOS project scaffold under `ios/`, created so Xcode opens/builds independently from `ios/`.
- Scyra app identity: display name, bundle target, primary brand color, basic AppIcon/LaunchScreen placeholder from verified sources or safe placeholders.
- Scyra design system basics: primary colors, typography fallback, card/button/chip styles, empty/loading/error states, custom Scyra top bar.
- Root SwiftUI app shell and typed navigation route model.
...
- Base Scyra Score calculation and timed bonus parity from Android.
- Reward reveal custom surface/deck, not a generic alert.
- Local persistence and repository layer for core Flow/Journey/Pulse/session/reward state.
- Settings needed for MVP: score visibility/defaults, movement toggle placeholder if HealthKit is deferred, language placeholder if localization is deferred.
- Notification/deep-link return to active Flow if scoped for MVP; otherwise explicit TODO.
- Basic Shell entry if reward reveal or creature rewards need a destination.
- The Chest creature-only inventory if creature rewards are included in MVP.
- Core badges if reward reveal includes badge grants.
- Minimum accessibility labels for core interactive flows, icon-only buttons, reward cards, and custom drawings.

### Should Have Soon After MVP

- HealthKit Movement Points and permission UX.
- Delayed movement refresh and idempotent Pearl deltas.
- The Blue visual room and procedural/native creature drawing strategy.
- Full Shell hub/room feeling.
- Arc/Paths planning and Arc multiplier integration.
- Lookout objectives, Voyage Hall stats, Focus Room with AVSpeechSynthesizer, Stillwater drops/draws.
- iOS notification polish, notification categories/actions if useful, and optional Live Activity decision.
- Spanish/Hindi/Marathi localization review, screenshot tests, and Devanagari typography fallback validation.

### Defer / Product Decision Required

- Aera flavor/support.
- Beam scheduling/reward behavior.
- Legacy objects/trinkets/discoveries and Discovery Journal.
- Room object placement, Coral, Plants, and non-creature inventory categories.
- Soundscapes/audio assets; no Android `res/raw` assets were found.
- Full Beyond Blue if catalog/design/scope is not MVP.
- Android-to-iOS data import and future cloud sync.
- Watch app, widgets, Live Activities, and full App Store marketing assets.
- Advanced Health metrics beyond steps such as heart rate, distance, workouts, or exercise sessions.
...
| -------- | -------------- | ------------------------ | --------------------------- | ------ |
| SwiftData vs SQLite | Determines schema control, migrations, transaction guarantees, test stores. | Android Room v31 with complex ledgers/entities. | Engineering architecture decision/spike. | Persistence foundation, rewards, ledgers. |
...
| Movement ratio | Reward parity may conflict with product direction. | Android uses `steps / 100`; prior product direction may prefer `1 point per 25 steps`. | Product + rewards owner. | HealthKit Movement, reward tests. |
| Surge final-score/Pearl contribution | Affects score, Pearls, reward reveal, economy inflation. | Android has Surge fields/formulas documented but ambiguity remains. | Rewards spec follow-up. | Reward engine, Pearl ledger. |
...
| Caveat font licensing | Brand typography cannot ship without rights. | Android has `caveatsb.ttf`; provenance unknown. | Design/legal/provenance follow-up. | Typography, brand title. |
| Turtle/logo provenance | Core app icon/top bar/Shell brand asset. | Android has `scyra_turtle.png`; provenance unknown. | Design/legal/provenance follow-up. | AppIcon, Shell icon, launch branding. |
| Scyra blue vs Android teal drift | iOS primary palette must be intentional. | Scyra primary is `0xFF2F4F6F`; Android Material primary may still use teal. | Design/product decision. | Design system foundation. |
...
| Arc/Paths MVP vs Phase 2 | Arc affects Flow launch and reward multipliers. | Android implements Paths/Arc routes and persistence. | Product sequencing decision. | Arc UI, reward engine integration. |
| Shell rooms MVP vs Phase 2 | Shell scope can expand MVP substantially. | Android has Shell root, rooms, Chest, Blue, Badges, Stillwater, Grove, Lookout, Voyage, Focus. | Product sequencing decision. | Shell PR sequence. |
| Lookout/Voyage/Focus/Stillwater MVP vs later | Each adds persistence/UI/reward complexity. | Android implements them; docs mark high-risk areas. | Product sequencing decision. | Phase 2 planning. |
...
- `Persistence/` owns SwiftData/SQLite models, migrations, repositories, transactions/unit-of-work, and in-memory test stores.
...
| Feature/module | Android reference docs/source | Future iOS folders | MVP priority | Dependencies | Major risks |
| -------------- | ----------------------------- | ------------------ | ------------ | ------------ | ----------- |
| App shell/startup | Docs 03, 08; `MainActivity`, `SkillzApplication` | `App/` | MVP | Project scaffold, DI | Launch/route restoration. |
| Design system | Doc 04; theme/resources/top bars | `DesignSystem/` | MVP | Color/font decisions | Scyra blue/teal drift, font license. |
| Navigation/root routes | Doc 03; `SkillzNavHost` | `App/Navigation`, `Features/*` | MVP | App shell | Route payload complexity. |
| Flow | Docs 03, 05, 06; `FlowViewModel`, Flow UI | `Features/Flow`, `Domain/Flow` | MVP | Persistence, reward basics | Timer/restoration correctness. |
| Ongoing Flow restoration | Docs 05, 08; `OngoingSessionEntity`, service | `Services/FlowLifecycle` | MVP | Persistence | iOS background limits. |
| Journey | Docs 02, 05; `TagEntity`, repositories | `Domain/Journey`, `Features/Journey` | MVP | Persistence | Android internal Tag naming. |
| Story/Chronicle | Docs 03, 04 | `Features/Story` | MVP | Flow/Pulse persistence | Naming Story/Chronicle/Horizon. |
| Pulse | Docs 02, 03, 05 | `Features/Pulse`, `Domain/Pulse` | MVP | Journey, persistence | Avoid reward behavior. |
| Reward engine | Doc 06 | `Domain/Rewards` | MVP | Domain models | Formula/idempotency decisions. |
| Reward reveal | Docs 03, 04, 06 | `Features/Rewards`, `DesignSystem` | MVP | Reward engine | Historical drift; custom UI. |
| Persistence | Doc 05 | `Persistence/` | MVP | SwiftData/SQLite decision | Transactions/migrations. |
| Settings | Docs 05, 08 | `Services/Settings`, `Features/Settings` | MVP | Persistence/settings store | Flavor/language choices. |
| Notifications/deep links | Docs 03, 08 | `Services/Notifications`, `Services/DeepLinks` | MVP/soon | Active Flow restoration | No foreground-service equivalent. |
| HealthKit Movement | Docs 06, 08 | `Services/Health`, `Domain/Movement` | Soon after MVP unless scoped | Reward engine, permissions | HealthKit timing/ratio decision. |
| Shell root | Docs 03, 04 | `Features/Shell` | MVP if rewards need Shell; otherwise Phase 2 | Design system | Scope creep. |
| The Chest | Docs 02, 03, 05, 06 | `Features/Shell/Chest` | MVP if creature rewards included | Creature persistence | Creature-only direction vs legacy. |
| Badges | Docs 05, 06 | `Features/Shell/Badges` | MVP if badge rewards included | Badge persistence/service | Mastery idempotency. |
...
| Notepad | Docs 03, 05 | `Features/Notepad` | Later/MVP if root nav requires | Settings/editor decision | Rich editor parity. |
| Localization | Docs 07, 08 | `Resources/Localizable.xcstrings`, `Services/Locale` | MVP base; locales later | String Catalog setup | Layout/translation quality. |
| Assets/resources | Doc 07 | `Resources/`, `Assets.xcassets` | MVP basics | Provenance decisions | License/source gaps. |
| Tests | Docs 01, 05, 06, 08 | `ScyraTests`, `ScyraUITests` | MVP | Architecture in place | Coverage discipline. |
...
| ------- | ----- | ---- | -------------------- | --------------------------- | ------------ | ------------------- | ---- | ----- | ----------------- |
| IOS-00 | Create iOS project scaffold | Manually create Xcode project under `ios/`. | Doc 00 | `ios/Scyra.xcodeproj`, `ios/Scyra/`, tests | Human/Xcode decision | Xcode opens/builds from `ios/`; Android untouched. | Medium | MVP | Manual Xcode step / human decision first. |
| IOS-01 | Add app root and dependency container | Create `@main`, root view, dependency container skeleton. | Docs 03, 08 | `App/`, `Services/`, `Features/Home` | IOS-00 | App boots to placeholder root; container injectable. | Medium | MVP | Good for Codex after project exists. |
| IOS-02 | Add folder conventions and Scyra identity | Establish groups, display name, safe placeholders. | Docs 00, 07, 08 | `Resources/`, `Assets.xcassets` | IOS-00 | iOS remains self-contained; no Android asset refs. | Medium | MVP | Good for Codex with asset decisions. |
| IOS-03 | Design system foundation | Add Scyra color tokens, spacing, card/button/chip primitives. | Docs 04, 07 | `DesignSystem/` | Scyra blue decision | Components preview/testable; no font copy unless licensed. | Medium | MVP | Good for Codex. |
| IOS-04 | Typography/top bar foundation | Add top bar, typography fallback, title treatment. | Docs 04, 07 | `DesignSystem/Typography`, `ScyraTopBar` | Font/license decision for custom font | Top bar order and labels match docs. | Medium | MVP | Good for Codex; font copy blocked. |
| IOS-05 | Domain model foundation | Define Flow, Journey, Pulse, session, reward, ledger domain models. | Docs 02, 05, 06 | `Domain/Models` | Architecture decisions | Models are product-clean and Android legacy names are avoided. | Medium | MVP | Good for Codex. |
| IOS-06 | Persistence backend decision | Choose SwiftData or SQLite and document/implement container skeleton. | Doc 05 | `Persistence/` | Human architecture decision | Persistence container builds; migrations plan exists. | High | MVP | Needs human decision first. |
| IOS-07 | Repository protocols + in-memory stores | Add repository protocols and test stores. | Docs 05, 06 | `Persistence/Repositories`, tests | IOS-05 | Tests can run without real DB. | Medium | MVP | Good for Codex. |
| IOS-08 | Settings repository | Implement score/calm/language/movement settings wrapper. | Docs 05, 08 | `Services/Settings` | IOS-07 | Settings persist locally and are test-backed. | Low | MVP | Good for Codex. |
| IOS-09 | Flow lifecycle | Implement start/pause/resume/complete domain/service. | Docs 03, 05, 08 | `Domain/Flow`, `Services/FlowLifecycle` | IOS-07 | Timestamp-based elapsed time tests pass. | High | MVP | Good for Codex. |
| IOS-10 | Ongoing Flow restoration | Restore active Flow after background/relaunch. | Docs 05, 08 | Flow lifecycle + persistence | IOS-09, persistence | Relaunch tests reconstruct elapsed time. | High | MVP | Good for Codex. |
| IOS-11 | Journey + Story basics | Implement Journey selection and Story/Chronicle list. | Docs 03, 05 | `Features/Story`, `Domain/Journey` | IOS-07, IOS-09 | Completed Flows appear in history. | Medium | MVP | Good for Codex. |
| IOS-12 | Pulse capture/edit/history | Implement Pulse as idea/thought, no rewards. | Docs 02, 03, 05, 06 | `Features/Pulse` | IOS-07, Journey | Pulse tests prove no Score/Pearls. | Medium | MVP | Good for Codex. |
| IOS-13 | Base reward calculator | Implement Scyra Score/timed bonuses. | Doc 06 | `Domain/Rewards`, tests | IOS-05 | Android parity formula tests pass. | High | MVP | Good for Codex. |
| IOS-14 | Reward breakdown + Pearl ledger | Persist reward breakdown and ledger-backed Pearls. | Docs 05, 06 | `Domain/Economy`, `Persistence` | IOS-06, IOS-13 | Ledger/idempotency tests pass. | High | MVP | Good for Codex. |
| IOS-15 | Reward reveal UI | Build custom reward deck/cards. | Docs 03, 04, 06 | `Features/Rewards`, `DesignSystem` | IOS-13 | No generic alert; mapper tests pass. | Medium | MVP | Good for Codex. |
| IOS-16 | Creature/badge basics | Add regular Flow creature drops and basic badges if MVP includes them. | Docs 05, 06, 07 | `Domain/Creatures`, `Features/Shell/Chest`, `Badges` | Creature scope decision | Chest groups creatures only; no released/legacy objects. | High | MVP/Phase 2 | Good after decision. |
| IOS-17 | Notifications and deep links | Add permission education, local notifications, route handling. | Docs 03, 08 | `Services/Notifications`, `DeepLinks` | IOS-10 | Notification tap restores active Flow. | High | MVP/soon | Good for Codex; prompt timing decision needed. |
| IOS-18 | HealthKit Movement | Add step authorization/query and movement points. | Docs 06, 08 | `Services/Health`, tests | Movement ratio decision, IOS-13 | Step aggregation/idempotency tests pass. | High | Soon | Good after decision. |
...
| IOS-20 | Shell root | Build basic Shell hub/entry. | Docs 03, 04 | `Features/Shell` | Design system | Shell feels like hub, not settings page. | Medium | MVP/Phase 2 | Good after scope. |
| IOS-21 | The Chest | Creature-only inventory grid. | Docs 02, 03, 04, 05, 07 | `Features/Shell/Chest` | Creature persistence | Level/count grouping works. | Medium | MVP/Phase 2 | Good for Codex. |
| IOS-22 | Badges and Mastery | Badge screen and Mastery idempotency. | Docs 05, 06 | `Features/Shell/Badges` | Mastery decision | Countable Mastery cannot double-award. | High | Phase 2/MVP if badges included | Needs decision first. |
...
| PR 3: `ios/design-system-foundation` | Add colors, spacing, cards/buttons/chips. | `DesignSystem/` | Snapshot/unit where practical | Scyra blue/teal decision. | Primary color decision. |
...
| PR 6: `ios/persistence-foundation` | Add SwiftData/SQLite container. | `Persistence/` | Persistence smoke tests | High transaction stakes. | SwiftData vs SQLite. |
...
| PR 18: `ios/healthkit-movement` | Add HealthKit steps and movement reward refresh. | `Services/Health` | Health fake/idempotency tests | Ratio and background limits. | Movement ratio decision. |
...
## 11. MVP Dependency Graph

| Feature | Depends on | Blocks | Notes |
| ------- | ---------- | ------ | ----- |
| Project scaffold | Human Xcode setup | Everything iOS | Must live under `ios/`. |
| Design system | Scyra primary color decision, safe font fallback | All screens | Font file copy waits for license. |
| Persistence | SwiftData vs SQLite decision | Flow, Story, rewards, settings | Use repository layer either way. |
| Settings | Persistence/settings store | Score visibility, movement toggle, language | Keep behind service/repository. |
| Flow | Design system, persistence, settings | Story, rewards, notifications, movement | Core MVP. |
...
| Movement | HealthKit service, permission UI, reward engine | Movement Points/Pearl deltas | Blocked by ratio decision. |
...
| Risk | Severity | Source doc | Why it matters | Mitigation | Before MVP? |
| ---- | -------- | ---------- | -------------- | ---------- | ----------- |
...
| Movement ratio unresolved | High | Doc 06 | Android `steps / 100` may conflict with product direction. | Product decision before HealthKit. | If Movement MVP |
| Surge contribution unresolved | Medium/high | Doc 06 | Score/Pearl math can drift. | Rewards follow-up and tests. | Before Surge MVP |
...
| SwiftData vs SQLite decision | High | Doc 05 | Affects migrations, transactions, tests. | Architecture spike. | Yes |
| The Blue procedural rendering complexity | High | Docs 04, 07 | Hard to replicate Compose Canvas visuals. | Prototype SwiftUI Canvas/assets. | No, unless Blue MVP |
| Font licensing | Medium | Docs 04, 07 | Brand font cannot ship unverified. | License/provenance verification; fallback. | Before final branding |
| Turtle/logo provenance | Medium/high | Doc 07 | Core brand/AppIcon may be blocked. | Locate verified source artwork. | Before release |
...
| HealthKit permission/background limitations | High | Doc 08 | Delayed movement may not refresh exactly. | Foreground refresh + idempotency; BGTask only if proven. | If Movement MVP |
| Notification prompt timing | Medium | Doc 08 | Bad prompt timing reduces opt-in. | Permission education coordinator. | If notifications MVP |
...
| Scyra blue vs Android teal drift | Medium | Docs 04, 07 | Visual identity can drift. | Product/design color decision. | Yes |
...
| ------- | ---------------- | ------------------------- | ------------ | ---------------- |
| Beam | Strings/routes/reward references in docs and Android copy. | Current Scyra backlog emphasizes Flow/Pulse/Arc/Surge. | Do not port MVP. | Product decision if Beam returns. |
...
| Discovery Journal / discoveries | Entity/reward/source references and copy. | Not confirmed for iOS MVP. | Do not port MVP. | Product/data compatibility decision. |
| Trinkets | Reward event/source references. | The Chest creature-only. | Do not port. | Product confirmation required. |
| Room objects | `shell_object_*`, placement/source entities. | The Chest creature-only; Shell decorations not MVP. | Do not port MVP. | Shell room decoration decision. |
...
| Coral | `shell_category_coral`, Glow Coral copy. | Non-creature/legacy. | Defer/skip MVP. | Product confirmation. |
| Plants | `shell_category_plants`. | Non-creature/legacy. | Defer/skip MVP. | Product confirmation. |
| Object placement | Placement entities/copy. | Not MVP unless Shell decoration returns. | Skip MVP. | Product decision. |
...
| Non-step Health metrics | Android reads only `StepsRecord`. | Movement Points use steps for current parity. | Do not request heart/distance/workouts MVP. | Product decision for advanced health. |

## 14. Test Strategy for iOS Parity

| Test category | Source docs / Android tests to reference | Priority | MVP required or later |
| ------------- | ---------------------------------------- | -------- | --------------------- |
| Domain unit tests | Docs 05, 06 | High | MVP |
| Reward calculator tests | `ScoreCalculator`, reward spec/tests | High | MVP |
...
| Flow lifecycle/restoration tests | Docs 05, 08; `OngoingSessionEntity` | High | MVP |
| Persistence repository tests | Docs 05; Room migration/repository tests | High | MVP |
| Pearl ledger tests | Docs 05, 06; Shell economy tests | High | MVP if Pearls included |
...
| Pulse no-reward tests | Docs 02, 06 | High | MVP |
| Soft Flow no-score tests | Docs 02, 06 | High | MVP if Soft Flow included |
| Notification/deep-link routing tests | Docs 03, 08 | Medium/high | If notifications MVP |
...
| Localization snapshot/layout tests | Docs 07, 08 | Medium | Base MVP; full locales later |
| Accessibility tests | Docs 04, 07 | Medium/high | MVP core flows |
| UI tests for core flows | Navigation/UX docs | Medium | MVP smoke flows |
...
- Verify turtle logo provenance before using `scyra_turtle.png` equivalent in iOS.
- Verify Caveat font license before copying a font into `ios/` and registering it.
...
- Create Scyra color tokens, including Royal Manuscript Blue `0xFF2F4F6F`, and resolve Android teal drift.
...
- Add `PersistenceContainer` once SwiftData vs SQLite is decided.
...
- Resolve and implement Surge final-score/Pearl behavior.
- Implement Arc multiplier only after Arc scope and source-of-truth decisions.
- Resolve Movement ratio (`steps / 100` parity vs possible `1 point per 25 steps`).
- Implement ledger-backed Pearls.
- Implement delayed movement delta idempotency.
- Implement regular creature drops if creatures are in MVP.
...
- Movement ratio decision.
- Surge final-score/Pearl decision.
- Creature Mastery persistence/idempotency design.

## 18. Data / Persistence Backlog

- Choose SwiftData vs SQLite.
...
- MVP scope recommended: iOS scaffold, Scyra identity, design system basics, root navigation, Flow lifecycle/restoration, Journey, Story/Chronicle, Pulse, base reward calculator, reward reveal, local persistence, settings, optional notifications/deep links, and Shell/Chest/Badges only where needed by reward scope.
- Deferred scope recommended: HealthKit Movement if not MVP, The Blue, full Shell hub, Arc/Paths, Lookout, Voyage Hall, Focus Room TTS, Stillwater, localization review, Aera, Beam, legacy objects/trinkets/discoveries, soundscapes, data import, cloud sync, Watch app, Live Activities, advanced Health metrics.
- Biggest open decisions: SwiftData vs SQLite, Scyra-only vs Aera, Movement ratio, Surge reward contribution, Creature Mastery idempotency, font/logo provenance, Scyra blue vs teal drift, notification timing, backup/iCloud policy, iOS language override, The Blue drawing strategy, and Phase 2 room scope.
- Highest-risk iOS parity areas: active Flow restoration without Android foreground service, reward/economy idempotency, HealthKit delayed refresh, creature Mastery, The Blue procedural rendering, asset/font provenance, localization layout, App Store permissions/privacy.
