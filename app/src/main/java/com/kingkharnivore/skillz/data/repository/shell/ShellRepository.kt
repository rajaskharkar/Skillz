...
class ShellRepository @Inject constructor(
    ...
    suspend fun placeInstance(instanceId: String, roomId: ShellRoomId, slotId: String) = db.withTransaction {
        ...
        val slot = ShellContentCatalog.focusSlots.firstOrNull { it.roomId == roomId && it.slotId == slotId } ?: error("Slot not found")
        require(find.placeable) { "This Shell Find rests in the Shell Chest." }
        require(slot.slotType in find.acceptedSlotTypes && find.category in slot.acceptsCategories) { "This space is not right for that Shell Find." }
    ...
    suspend fun upgradeInstance(instanceId: String) = db.withTransaction {
        val instance = findInstanceDao.getById(instanceId) ?: error("Shell Find not found")
        val find = ShellContentCatalog.find(instance.findId) ?: error("Shell Find definition missing")
        require(find.upgradeable) { "This Shell Find is already as it wishes to be." }
        val next = ShellContentCatalog.nextUpgrade(find.findId, instance.currentUpgradeStageId) ?: error("This form is complete.")
        val balance = pearlLedgerDao.getBalance()
        require(balance >= next.pearlCost) { "More Pearls need to gather first." }
