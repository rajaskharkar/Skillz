...
interface PearlLedgerDao {
    @Query("SELECT COALESCE(SUM(delta), 0) FROM pearl_ledger") fun observeBalance(): Flow<Int>
    @Query("SELECT COALESCE(SUM(delta), 0) FROM pearl_ledger") suspend fun getBalance(): Int
    @Insert(onConflict = OnConflictStrategy.ABORT) suspend fun insert(entry: PearlLedgerEntity)
    @Query("SELECT COUNT(*) FROM pearl_ledger WHERE sourceType = :sourceType AND sourceId = :sourceId AND reason = :reason") suspend fun sourceRewardCount(sourceType: String, sourceId: String?, reason: String): Int
    @Query("SELECT * FROM pearl_ledger ORDER BY createdAt DESC LIMIT :limit") fun observeRecent(limit: Int = 20): Flow<List<PearlLedgerEntity>>
}

@Dao
interface ShellFindInstanceDao {
    @Insert(onConflict = OnConflictStrategy.ABORT) suspend fun insert(entity: UserShellFindInstanceEntity)
    @Update suspend fun update(entity: UserShellFindInstanceEntity)
    @Query("SELECT * FROM user_shell_find_instance ORDER BY acquiredAt DESC") fun observeAll(): Flow<List<UserShellFindInstanceEntity>>
    @Query("SELECT * FROM user_shell_find_instance WHERE instanceId = :instanceId LIMIT 1") suspend fun getById(instanceId: String): UserShellFindInstanceEntity?
    @Query("SELECT * FROM user_shell_find_instance WHERE findId = :findId LIMIT 1") suspend fun getFirstByFindId(findId: String): UserShellFindInstanceEntity?
    ...
    @Query("SELECT * FROM user_shell_find_instance WHERE findId = :findId AND animalLevel = :level AND creatureStatus = :status ORDER BY acquiredAt ASC, instanceId ASC") suspend fun getActiveByFindIdAndLevel(findId: String, level: Int, status: String): List<UserShellFindInstanceEntity>
}
...
interface ShellPlacementDao {
    @Insert(onConflict = OnConflictStrategy.ABORT) suspend fun insert(entity: ShellPlacementEntity)
    @Query("DELETE FROM shell_placement WHERE instanceId = :instanceId") suspend fun removeByInstance(instanceId: String)
    @Query("DELETE FROM shell_placement WHERE placementId = :placementId") suspend fun remove(placementId: String)
    @Query("SELECT * FROM shell_placement WHERE roomId = :roomId ORDER BY placedAt DESC") fun observeByRoom(roomId: String): Flow<List<ShellPlacementEntity>>
    @Query("SELECT * FROM shell_placement WHERE roomId = :roomId") suspend fun getByRoom(roomId: String): List<ShellPlacementEntity>
    @Query("SELECT * FROM shell_placement WHERE slotId = :slotId AND roomId = :roomId LIMIT 1") suspend fun getBySlot(roomId: String, slotId: String): ShellPlacementEntity?
    @Query("SELECT * FROM shell_placement WHERE instanceId = :instanceId LIMIT 1") suspend fun getByInstance(instanceId: String): ShellPlacementEntity?
}

@Dao
interface ShellFindUpgradeDao {
    @Insert(onConflict = OnConflictStrategy.ABORT) suspend fun insert(entity: ShellFindUpgradeEntity)
    @Query("SELECT * FROM shell_find_upgrade WHERE instanceId = :instanceId ORDER BY upgradedAt DESC") suspend fun getForInstance(instanceId: String): List<ShellFindUpgradeEntity>
}
...
interface UserDiscoveryDao {
    @Insert(onConflict = OnConflictStrategy.ABORT) suspend fun insert(entity: UserDiscoveryEntity)
...
interface StillwaterPreferenceDao {
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun upsert(entity: StillwaterPreferenceEntity)
    @Query("SELECT * FROM stillwater_preference WHERE id = 1 LIMIT 1") fun observe(): Flow<StillwaterPreferenceEntity?>
    @Query("SELECT * FROM stillwater_preference WHERE id = 1 LIMIT 1") suspend fun get(): StillwaterPreferenceEntity?
}
...
interface UserShellRoomStateDao {
    ...
    @Query("SELECT * FROM user_shell_room_state") fun observeAll(): Flow<List<UserShellRoomStateEntity>>
}
