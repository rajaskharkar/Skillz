...
class ObjectiveProgressCalculator @Inject constructor() {
    ...
    fun initialWindow(startAtMs: Long, period: ObjectivePeriod, weeklyBoundaryDay: Int?, zoneId: ZoneId): ObjectiveWindow {
        ...
        val end = when (period) {
            ...
            ObjectivePeriod.Weekly -> {
                val boundary = DayOfWeek.of(weeklyBoundaryDay ?: start.dayOfWeek.value)
                val days = ((boundary.value - start.dayOfWeek.value + 7) % 7).let { if (it == 0) 7 else it }
