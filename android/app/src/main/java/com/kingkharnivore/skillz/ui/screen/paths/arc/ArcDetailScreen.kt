...
fun ArcDetailScreen(
    ...
    val completeCurrentFlowText = stringResource(R.string.arc_detail_complete_current_flow)
    val snackbarHostState = remember { SnackbarHostState() }

    LaunchedEffect(uiState.launchBlockedByActiveFlow) {
        if (uiState.launchBlockedByActiveFlow) {
            snackbarHostState.showSnackbar(completeCurrentFlowText)
            viewModel.clearLaunchBlockedMessage()
        }
    }
